using Application.DTO.Request;
using Application.DTO.Response;
using FluentAssertions;
using Infrastructure.Data.Configuration;
using Microsoft.AspNetCore.TestHost;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Hosting;
using System.Net;
using System.Net.Http;
using Tests.Integration.Base;
using Xbehave;

namespace Tests.Integration.Scenarios
{
    public class Login : BaseScenario
    {
        [Scenario]
        public void After_Signup_User_Should_Be_Able_To_Login()
        {
            IHost host = null;
            HttpResponseMessage response = null;
            var signupRequest = new SignupRequest
            {
                Login = "foo",
                Password = "bar1234569009"
            };
            var loginRequest = new LoginRequest
            {
                Login = signupRequest.Login,
                Password = signupRequest.Password
            };

            "Given the host".x(async stepContext => host = await GetHost());
            "And given there is no user initially, add a user".x(async stepContext =>
            {
                using var scope = host.Services.CreateScope();
                var dbContext =
                    (AuthDbContext)(scope.ServiceProvider.GetService(typeof(AuthDbContext)));

                (await dbContext.Users
                    .AsNoTracking()
                    .CountAsync()).Should().Be(0);

                await host.GetTestClient().PostAsync("signup", GetStringContent(signupRequest));
            });
            "When a POST is made".x(async stepContext =>
            {
                response = await host.GetTestClient().PostAsync("login", GetStringContent(loginRequest));
            });
            "Then the status OK is returned".x(stepContext =>
            {
                response.Should().NotBeNull();
                response.StatusCode.Should().Be(HttpStatusCode.OK);
            });
            "And the token is returned as well".x(async stepContext =>
            {
                var signupResponse = await DeserializeResponseContent<LoginResponse>(response);
                signupResponse.Success.Should().BeTrue();
                signupResponse.Message.Should().BeNull();
                signupResponse.Token.Should().NotBeNullOrEmpty();
            });
        }

        [Scenario]
        public void Login_And_Password_Must_Be_Present()
        {
            IHost host = null;
            var loginRequest = new LoginRequest
            {
                Login = "",
                Password = "bar1234569009"
            };

            void SomethingIsNotPresent(LoginRequest request)
            {
                HttpResponseMessage response = null;

                "And given there is no user initially".x(async stepContext =>
                {
                    using var scope = host.Services.CreateScope();
                    var dbContext =
                        (AuthDbContext)(scope.ServiceProvider.GetService(typeof(AuthDbContext)));

                    (await dbContext.Users
                        .AsNoTracking()
                        .CountAsync()).Should().Be(0);
                });
                "When a POST is made".x(async stepContext =>
                {
                    response = await host.GetTestClient().PostAsync("login", GetStringContent(request));
                });
                "Then the status OK is returned".x(stepContext =>
                {
                    response.Should().NotBeNull();
                    response.StatusCode.Should().Be(HttpStatusCode.OK);
                });
                "And the token is not returned".x(async stepContext =>
                {
                    var signupResponse = await DeserializeResponseContent<SignupResponse>(response);
                    signupResponse.Success.Should().BeFalse();
                    signupResponse.Message.Should().Be("Login and password must be present");
                    signupResponse.Token.Should().BeNull();
                });
            }

            "Given the host".x(async stepContext => host = await GetHost());
            SomethingIsNotPresent(loginRequest);

            loginRequest.Login = "foo";
            loginRequest.Password = null;
            SomethingIsNotPresent(loginRequest);
        }

        [Scenario]
        public void Max_Login_Length_Is_10()
        {
            IHost host = null;
            HttpResponseMessage response = null;
            var signupRequest = new LoginRequest
            {
                Login = "1234567890-",
                Password = "bar1234569009"
            };

            "Given the host".x(async stepContext => host = await GetHost());
            "And given there is no user initially".x(async stepContext =>
            {
                using var scope = host.Services.CreateScope();
                var dbContext =
                    (AuthDbContext)(scope.ServiceProvider.GetService(typeof(AuthDbContext)));

                (await dbContext.Users
                    .AsNoTracking()
                    .CountAsync()).Should().Be(0);
            });
            "When a POST is made".x(async stepContext =>
            {
                response = await host.GetTestClient().PostAsync("login", GetStringContent(signupRequest));
            });
            "Then the status OK is returned".x(stepContext =>
            {
                response.Should().NotBeNull();
                response.StatusCode.Should().Be(HttpStatusCode.OK);
            });
            "And the token is not returned".x(async stepContext =>
            {
                var signupResponse = await DeserializeResponseContent<SignupResponse>(response);
                signupResponse.Success.Should().BeFalse();
                signupResponse.Message.Should().Be("Max login length is 10");
                signupResponse.Token.Should().BeNull();
            });
        }

        [Scenario]
        public void Wrong_Password_Should_Not_Give_Token()
        {
            IHost host = null;
            HttpResponseMessage response = null;
            var loginRequest = new LoginRequest
            {
                Login = "foo",
                Password = "bar12345678899"
            };

            "Given the host".x(async stepContext => host = await GetHost());
            "And given there is no user initially, add a user".x(async stepContext =>
            {
                using var scope = host.Services.CreateScope();
                var dbContext =
                    (AuthDbContext)(scope.ServiceProvider.GetService(typeof(AuthDbContext)));

                (await dbContext.Users
                    .AsNoTracking()
                    .CountAsync()).Should().Be(0);

                await host.GetTestClient().PostAsync("signup", GetStringContent(
                    new SignupRequest { Login = loginRequest.Login, Password = loginRequest.Password }
                ));
            });
            "When a POST is made  with a wrong password".x(async stepContext =>
            {
                loginRequest.Password += "1";
                response = await host.GetTestClient().PostAsync("login", GetStringContent(loginRequest));
            });
            "Then the status OK is returned".x(stepContext =>
            {
                response.Should().NotBeNull();
                response.StatusCode.Should().Be(HttpStatusCode.OK);
            });
            "And the token is not returned".x(async stepContext =>
            {
                var signupResponse = await DeserializeResponseContent<LoginResponse>(response);
                signupResponse.Success.Should().BeFalse();
                signupResponse.Message.Should().Be("Either Login or Password is incorrect");
                signupResponse.Token.Should().BeNull();
            });
        }
    }
}
